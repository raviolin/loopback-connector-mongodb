...
/*!
 * Convert the id to be a BSON UUID if it is compatible
 * @param {*} id The id value
 * @returns {UUID}
 */
function UUID(id) {
  ...
  return new mongodb.Binary(Buffer.from(id.trim().replace(/-/g, ''), 'hex'), 4);
}

/*!
 * Convert BSON UUID to a string representation
 * @param {UUID} id The id value
 * @returns {String}
 */
function UUIDString(id) {
  return [[0, 4], [4, 2], [6, 2], [8, 2], [10, 6]].map(function(i) {
    return id.read(i[0], i[1]).toString('hex');
  }).join('-');
}
...
MongoDB.prototype.fromDatabase = function(model, data) {
  ...
  for (var p in props) {
    var prop = props[p];
    if (prop && prop.type === String && prop.uuid === true) {
      if (data[p] instanceof mongodb.Binary) {
        data[p] = UUIDString(data[p]);
      }
    } else if (prop && prop.type === Buffer) {
      if (data[p] instanceof mongodb.Binary) {
...
MongoDB.prototype.coerceId = function(model, id, options) {
  ...
  if (idProp && typeof idProp.type === 'function') {
    if (idProp.uuid === true && typeof idValue === 'string') {
      idValue = UUID(idValue);
    } else if (!(idValue instanceof idProp.type)) {
...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  this.execute(model, 'insertOne', data, {safe: true}, function(err, result) {
    ...
    if (!self.isUUIDProperty(model, idName, idValue)) {
    ...
    if (spec) {
        ...
        query[k] = {
          $in: cond.map(function(x) {
            if (self.isObjectIDProperty(model, prop, x, options))
              return ObjectID(x);
            if (self.isUUIDProperty(model, prop, x))
              return UUID(x);
            return x;
          }),
        };
      ...
      if (cond === null) {
        ...
        if (self.isObjectIDProperty(model, prop, cond, options)) {
          cond = ObjectID(cond);
        } else if (self.isUUIDProperty(model, prop, cond)) {
          cond = UUID(cond);
        }
        query[k] = cond;
      }
...
/**
 * Check whether the property is an UUID
 *
 */
MongoDB.prototype.isUUIDProperty = function(model, prop, value) {
  if (prop && prop.uuid === true && prop.type === String) {
    return true;
  } else if ('string' === typeof value) {
    return /^\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}$/.test(value);
  } else {
    return false;
  }
};
